<template>
  <div class="page-container">
    ...
    <div class="admin-cards">
      ...
      <div class="admin-card card" @click="$router.push('/analytics')">
        <div class="admin-icon">📊</div>
        <h3>数据看板</h3>
        <p>查看平台整体数据统计</p>
      </div>
...
async function loadCodes() {
  ...
  try {
    ...
    if (res.code === 200) {
      ...
      unusedCount.value = res.data.unused || 0
    }
...
async function generateCodes() {
  ...
  try {
    ...
    if (res.code === 200) {
      ElMessage.success(`成功生成 ${res.data.generated} 个邀请码`)
      loadCodes()
    }
...
async function deleteCode(row: any) {
  ...
  try {
    const res: any = await api.delete(`/admin/invite-codes/${row.id}`)
    if (res.code === 200) { ElMessage.success('已删除'); loadCodes() }
  } catch {}
}
