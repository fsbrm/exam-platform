...
public interface UserAnswerMapper extends BaseMapper<UserAnswer> {
    ...
    @Select("SELECT u.id, u.username, u.nickname, COUNT(*) as total, " +
            "CAST(SUM(CASE WHEN ua.is_correct=1 THEN 1 ELSE 0 END)*100.0/COUNT(*) AS DECIMAL(5,1)) as accuracy, " +
            "CAST(SUM(CASE WHEN ua.is_correct=1 THEN 1 ELSE 0 END)*100.0/COUNT(*)*LOG(COUNT(*)+1) AS DECIMAL(5,1)) as cnt " +
            "FROM user_answer ua JOIN user u ON ua.user_id=u.id " +
            "WHERE DATE(ua.answered_at)=CURDATE() GROUP BY u.id,u.username,u.nickname " +
            "HAVING COUNT(*)>=3 ORDER BY cnt DESC LIMIT #{limit}")
